...
chain = np.load(join(data_dir, 'data', tipe, f'chain0-{model}-{data}.npy'))
p0 = chain[-1]

t0 = time()
chain = dm.mcmc(10000, nwalkers, p0, zdata, wdata, locs, scales, dz=1, verbose=True, parallel=parallel)
print(time() - t0, "s")
print(chain.shape)
np.save(join(data_dir, 'data', tipe, f'chain-{model}-{data}.npy'), chain)