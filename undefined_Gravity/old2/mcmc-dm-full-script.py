...
from utils import safe_mkdir, nu_mod, fw
...
asset_dir = join(data_dir, "assets-dm-full-"+name[:-5])
...
# ================================================
# probs function
from utils import log_posterior_dm
# ================================================
# utils functions
from utils import (load_data, plot_data, initialize_prior_dm,
                   initialize_walkers_dm, run_mcmc, consume_samples_dm,
                   plot_corner, plot_fitting_dm, get_dataframe_dm)
# ************************************************
# ************************************************
# ************************************************
# main program
...
## initialize prior
locs, scales, uni_list, norm_list = initialize_prior_dm(dim, w0, sigma_w, a_raw, simple=simple)
# initialize walkers
p0, ndim, nwalkers = initialize_walkers_dm(locs, scales, dim, simple=simple)
sampler = run_mcmc(
    nwalkers, ndim, p0, dim, log_posterior_dm, consume_samples_dm, 
...
sampler_new = run_mcmc(
    nwalkers, ndim, next_p0, dim, log_posterior_dm, consume_samples_dm, 
    args=(data, locs, scales, dim, norm_list, uni_list, simple), 
    cores=16, plot=True, step=step, simple=simple, path=join(asset_dir, "chain-plot-1.png")
)
# plot corner
samples = sampler_new.get_chain()
plot_corner(samples, dim, consume_samples_dm, simple=simple, path=join(asset_dir, "corner-plot.png"))
# plot fitting
plot_fitting_dm(sampler_new, data, dim, simple=simple, n=500, path=join(asset_dir, "fitting-plot.png"))
# convert to dataframe
df = get_dataframe_dm(sampler_new, dim, locs, scales, norm_list, uni_list, nwalkers, simple=simple)
print(df.shape)
# save to hdf5
df.export(join(data_dir, f"no_{full}_{name}"), progress=True)