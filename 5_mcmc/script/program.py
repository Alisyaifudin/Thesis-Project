...
result_paths = {
    'DM': join(root_data_dir, 'MCMC', 'dm'),
    'DDDM': join(root_data_dir, 'MCMC', 'dddm'),
    'NO': join(root_data_dir, 'MCMC', 'no'),
    'MOND': join(root_data_dir, 'MCMC', 'mond')
}
z_dir_path = join(root_data_dir, 'Vertical-Number')
w_dir_path = join(root_data_dir, 'Velocity-Distribution')
...
class Program:
    ...
    def plot_corner(self, args):
        ...
        print(f'\tLoading chain from\n\t{chain_path}')
        output_path_z = join(
            self.props['result_path'], 'plots', f'corner-z-{name}.pdf')
        output_path_w = join(
            self.props['result_path'], 'plots', f'corner-w-{name}.pdf')
        init = generate_init(self.props['model'], log_z_max, log_a_max)
        indexes = init['indexes']
        labs = init['labs']
        labels = init['labels']
