...
def run_mcmc(args):
    ...
    output_path = join(root_data_path, 'MCMC', 'dm', 'data')
...
def plot_chain(args):
    i = validate_args(args)
    chain = np.load(join(root_data_path, 'MCMC', 'dm', 'data', f'chain-2-{i}.npy'))
    output_path = join(root_data_path, 'MCMC', 'dm', 'plots')
...
def calculate_prob(args):
    i = validate_args(args)
    chain = np.load(join(root_data_path, 'MCMC', 'dm', 'data', f'chain-2-{i}.npy'))
    output_file = join(root_data_path, 'MCMC', 'dm', f'stats.txt')
    zpath = join(root_data_path, 'Effective-Volume')
    wpath = join(root_data_path, 'Velocity-Distribution')
    data = get_data(zpath, wpath, i)
