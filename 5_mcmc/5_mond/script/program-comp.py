...
def run_mcmc(args):
    ...
    output_path = join(root_data_path, 'MCMC', 'mond', 'comp', 'data')
    mcmc(mond, labs, indexes, data, i, steps0=1_500,
...
def plot_chain(args):
    i = validate_args(args)
    chain = np.load(join(root_data_path, 'MCMC', 'mond', 'comp', 'data', f'chain-2-{i}.npy'))
    output_path = join(root_data_path, 'MCMC', 'mond', 'comp', 'plots')
...
def plot_fit(args):
    ...
    zdata, wdata = data
    output_path = join(root_data_path, 'MCMC', 'mond', 'comp', 'plots')
    pfit(mond, zdata, wdata, chain, ndim, path=join(output_path, f'fit-2-{i}.pdf'))
...
def calculate_prob(args):
    i = validate_args(args)
    chain = np.load(join(root_data_path, 'MCMC', 'mond', 'comp', 'data', f'chain-2-{i}.npy'))
    output_file = join(root_data_path, 'MCMC', 'mond', 'comp', f'stats.txt')
    zpath = join(root_data_path, 'MCMC', 'dm_mock', 'mock', 'comp')
    wpath = zpath
    data = get_data(zpath, wpath, i)
    run_calculate_bic_aic(mond, labs, data, i, chain, output_file)
