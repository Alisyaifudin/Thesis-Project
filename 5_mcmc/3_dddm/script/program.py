...
labels = [r'$\rho_b\times 10^2$', r'$\rho_{\textup{DM}}\times 10^2$', r"$\sigma_{\textup{DD}}$", 
          ...
          r'$w_0$', r'$\log \sigma_{w1}$', r'$\log a_1$', r'$\log \sigma_{w2}$', 
          r'$\log a_2$']
labs = ['rhob', 'rhoDM', 'sigmaDD', 'log_hDD', 'log_nu0', 'R', 'zsun', 'w0', 'log_sigmaw1', 
...
def run_mcmc(args):
    ...
    output_path = join(root_data_path, 'MCMC', 'dddm', 'data')
    mcmc(dddm, labs, indexes, data, i, steps0=1_500,
...
def plot_chain(args):
    i = validate_args(args)
    chain = np.load(join(root_data_path, 'MCMC', 'dddm', 'data', f'chain-2-{i}.npy'))
    output_path = join(root_data_path, 'MCMC', 'dddm', 'plots')
...
def plot_fit(args):
    ...
    zdata, wdata = data
    output_path = join(root_data_path, 'MCMC', 'dddm', 'plots')
    pfit(dddm, zdata, wdata, chain, ndim, path=join(output_path, f'fit-2-{i}.pdf'))
...
def calculate_prob(args):
    i = validate_args(args)
    chain = np.load(join(root_data_path, 'MCMC', 'dddm', 'data', f'chain-2-{i}.npy'))
    output_file = join(root_data_path, 'MCMC', 'dddm', f'stats.txt')
    zpath = join(root_data_path, 'Effective-Volume')
    wpath = join(root_data_path, 'Velocity-Distribution')
    data = get_data(zpath, wpath, i)
    run_calculate_bic_aic(dddm, labs, data, i, chain, output_file)
