...
full = "simple" if simple else "full"
# ************************************************
# ************************************************
# ************************************************
# dir paths
root_data_dir = abspath(join(root_dir, "Data"))
data_dir = join(root_data_dir, "MCMC-dddm")
...
asset_dir = join(data_dir, "assets-dd-dm-full-"+name[:-5])
...
# ================================================
# utils functions
from utils import (load_data, plot_data, initialize_prior_dd_dm,
                   initialize_walkers_dd_dm, run_mcmc, consume_samples_dd_dm,
                   plot_corner, plot_fitting_dd_dm, get_dataframe_dd_dm)
# ************************************************
# ************************************************
# ************************************************
# main program
...
## initialize prior
locs, scales, uni_list, norm_list = initialize_prior_dd_dm(dim, w0, sigma_w, a_raw, simple=simple)
# initialize walkers
p0, ndim, nwalkers = initialize_walkers_dd_dm(locs, scales, dim, simple=simple)
sampler = run_mcmc(
    nwalkers, ndim, p0, dim, log_posterior_dd_dm, consume_samples_dd_dm, 
...
sampler_new = run_mcmc(
    nwalkers, ndim, next_p0, dim, log_posterior_dd_dm, consume_samples_dd_dm, 
    args=(data, locs, scales, dim, norm_list, uni_list, simple), 
    cores=16, plot=True, step=step, simple=simple, path=join(asset_dir, "chain-plot-1.png")
)
# plot corner
samples = sampler_new.get_chain()
plot_corner(samples, dim, consume_samples_dd_dm, simple=simple, path=join(asset_dir, "corner-plot.png"))
# plot fitting
plot_fitting_dd_dm(sampler_new, data, dim, simple=simple, n=500, path=join(asset_dir, "fitting-plot.png"))
# convert to dataframe
df = get_dataframe_dd_dm(sampler_new, dim, locs, scales, norm_list, uni_list, nwalkers, simple=simple)
print(df.shape)
# save to hdf5
df.export(join(data_dir, f"dd-dm_{full}_{name}"), progress=True)