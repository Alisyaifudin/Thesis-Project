...
columns_tmass = ["ra", "dec","j_m", "k_m", "designation"]
columns_tmass_names = ["ra", "dec", "Jmag", "Kmag", "designation"]
...
decs = np.arange(dec_lower, dec_upper+0.1, 10).astype(int)
ddec = decs[1] - decs[0]
print("dec: ", decs)
...
for i, (ra0, ra1) in enumerate(zip(ras[:-1], ras[1:])):
    ...
    while j < len(decs) -1:
        ...
        """
        job_gaia = timeout(Gaia.launch_job, args=(query_gaia,), timeout_duration=600, path=text_path)
